...
const ALL_PATTERNS = [
  {
    ...
    description: 'Repeated failed authentication attempts on a single account within a short time window, indicating brute-force or credential stuffing attacks.',
    category: 'Credential',
    riskLevel: 'high',
    status: 'active',
    occurrences: 156,
  ...
  },
  {
    ...
    description: 'Transaction amount significantly deviates (≥3× standard deviation) from the user\'s historical average, suggesting account takeover or unauthorized use.',
  ...
  },
  {
    ...
    description: 'Transaction originates from a geographic location inconsistent with the user\'s registered address or recent login history, especially across international borders.',
    category: 'Location',
    riskLevel: 'medium',
    status: 'active',
    occurrences: 87,
  ...
  },
  {
    ...
    description: 'Multiple transactions executed in quick succession within a very short time window, characteristic of automated fraud scripts or compromised account exploitation.',
    category: 'Transaction',
    riskLevel: 'medium',
    status: 'active',
    occurrences: 65,
  ...
  },
  {
    ...
    description: 'Account accessed from a device fingerprint that has never been associated with the user, particularly concerning when combined with high-value transactions.',
    category: 'Device',
    riskLevel: 'low',
    status: 'active',
    occurrences: 54,
  ...
  },
  {
    ...
    description: 'Transactions occurring at unusual hours inconsistent with the user\'s established behavioral patterns (e.g., 2–5 AM local time for dormant accounts).',
    category: 'Behavioral',
    riskLevel: 'low',
    status: 'active',
    occurrences: 43,
  ...
  },
  {
    ...
    description: 'Transaction or login originating from an IP address that appears on known fraud databases, Tor exit nodes, or previously flagged sources.',
    category: 'Network',
    riskLevel: 'high',
    status: 'active',
    occurrences: 72,
  ...
  },
  {
    ...
    description: 'High-value transactions initiated by accounts less than 7 days old — a classic indicator of synthetic identity fraud or money mule accounts.',
    category: 'Behavioral',
    riskLevel: 'high',
    status: 'review',
    occurrences: 38,
  ...
  },
  {
    ...
    description: 'Funds being transferred to accounts with characteristics matching money mule profiles: newly created, infrequently used, or linked to previous fraud reports.',
    category: 'Transaction',
    riskLevel: 'medium',
    status: 'active',
    occurrences: 29,
  ...
  },
  {
    ...
    description: 'Behavioral signals suggesting the session token or credentials have been stolen and are being used by a different party than the legitimate account owner.',
    category: 'Credential',
    riskLevel: 'high',
    status: 'inactive',
    occurrences: 21,
...
const FraudPatterns = () => {
  ...
  const filtered = useMemo(() => {
    ...
    result.sort((a, b) => {
      switch (sortBy) {
        case 'occurrences_asc':  return a.occurrences - b.occurrences;
        case 'occurrences_desc': return b.occurrences - a.occurrences;
        case 'accuracy_desc':    return b.accuracy - a.accuracy;
        case 'name_asc':         return a.name.localeCompare(b.name);
        default:                 return b.occurrences - a.occurrences;
      }
    });
  ...
  return (
    <div className="fraud-patterns-page">
      ...
      <div className="fp-page-header">
        <div className="fp-header-left">
          ...
          <div>
            ...
            <p className="fp-subtitle">Detection rules, pattern analysis, and threat intelligence</p>
          </div>
        </div>
      ...
      {/* Content */}
      {viewMode === 'chart' ? (
        <PatternTrendChart patterns={filtered.length > 0 ? filtered : ALL_PATTERNS} />
      ) : (
        <>
          {filtered.length === 0 ? (
