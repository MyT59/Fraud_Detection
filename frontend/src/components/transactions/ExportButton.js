...
const ExportButton = ({ data, filename = 'transactions' }) => {
  ...
  const exportToCSV = () => {
    ...
    try {
      // CSV headers
      const headers = ['Transaction ID', 'User', 'Amount', 'Time', 'Location', 'Status'];
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      tableHTML += '<th>Transaction ID</th><th>User</th><th>Amount</th><th>Time</th><th>Location</th><th>Status</th>';
      ...
      {showDropdown && (
        <div className="export-dropdown">
          ...
          <button className="dropdown-item" onClick={exportToJSON}>
            ...
            JSON File
            <small className="text-muted d-block">JavaScript Object Notation</small>
          </button>
        </div>
      )}
    </div>
