...
// ── Fallback statis — persis sama dengan data sebelumnya ──────────────────
const STATIC_ACTIVITIES = [
  {
    id: 1, type: 'fraud_detected',
    title: 'High-Risk Transaction Blocked',
    description: 'TRX001234 automatically blocked by system',
    user: 'System', time: '2 min ago',
    icon: 'bi-shield-exclamation', color: 'red',
    details: { amount: 'Rp 25.000.000', user: 'USR12345' },
  },
  {
    id: 2, type: 'manual_review',
    title: 'Transaction Approved',
    description: 'TRX001230 approved after manual review',
    user: 'Admin User', time: '15 min ago',
    icon: 'bi-check-circle', color: 'green',
    details: { reviewTime: '3 minutes' },
  },
  {
    id: 3, type: 'rule_update',
    title: 'Fraud Rule Updated',
    description: 'Velocity check threshold increased to 10',
    user: 'Security Team', time: '1 hr ago',
    icon: 'bi-gear', color: 'blue',
  ...
  },
  {
    ...
    user: 'System', time: '3 hr ago',
    icon: 'bi-file-earmark-text', color: 'purple',
    details: { reportId: 'RPT0015' },
  },
  {
    id: 6, type: 'user_action',
    title: 'User Settings Updated',
    description: 'Email notifications enabled',
    user: 'Admin User', time: '5 hr ago',
    icon: 'bi-person-gear', color: 'gray',
    details: { setting: 'Notifications' },
  },
  {
    id: 7, type: 'fraud_detected',
    title: 'Suspicious Pattern Detected',
    description: 'Geographic anomaly detected in TRX001225',
    user: 'System', time: '6 hr ago',
    icon: 'bi-geo-alt', color: 'red',
  ...
  },
  {
    ...
    user: 'System', time: '8 hr ago',
    icon: 'bi-cpu', color: 'blue',
...
// ── Normalise API activity → komponen format ──────────────────────────────
const normaliseActivity = (a) => ({
  ...
  // map color dari string icon jika tidak ada
  color:       a.color || (
    a.type === 'fraud_detected' ? 'red'    :
    a.type === 'manual_review'  ? 'green'  :
    a.type === 'system'         ? 'blue'   :
    a.type === 'alert'          ? 'orange' :
    a.type === 'report'         ? 'purple' : 'gray'
  ),
  details:     a.details || {},
});
...
const ActivityTimeline = ({ activities }) => {
  ...
  // Gunakan data API; fallback ke statis; normalise shape
  const source = (activities && activities.length > 0)
  ...
  return (
    <div className="activity-timeline-card">
      ...
      {/* List — height terkunci 400px, overflow scroll */}
      <div
      ...
      >
        {filtered.length === 0 ? (
          <div style={{
            ...
            <i className="bi bi-clock-history" style={{ fontSize: '2rem', opacity: 0.4 }}></i>
            <p style={{ margin: 0, fontSize: '0.875rem' }}>No activities found</p>
          </div>
        ) : (
