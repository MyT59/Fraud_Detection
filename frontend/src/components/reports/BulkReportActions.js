...
const BulkReportActions = ({
  ...
  const handleBulkDelete = () => {
    if (
      window.confirm(
        `Delete ${selectedReports.length} selected reports? This action cannot be undone.`,
      )
    ) {
          ...
          <div className="selection-info">
            ...
            <div>
              ...
              <small className="text-muted">
                {selectedReports.filter((r) => r.status === "Completed").length}{" "}
                completed,{" "}
                {
                  selectedReports.filter((r) => r.status === "Processing")
                    .length
                }{" "}
                processing
              </small>
          ...
          <div className="bulk-actions-buttons">
            <div className="action-dropdown">
              <button
                ...
                disabled={
                  !selectedReports.some((r) => r.status === "Completed")
                }
              >
                <i className="bi bi-download me-1"></i>
                Download
                <i
                  className={`bi bi-chevron-${showDownloadOptions ? "up" : "down"} ms-1`}
                ></i>
              </button>
                ...
                <div className="action-dropdown-menu">
                  ...
                  <div className="dropdown-footer">
                    ...
                    >
                      ...
                      Download (
                      {
                        selectedReports.filter((r) => r.status === "Completed")
                          .length
                      }
                      )
                    </button>
