...
class Admin(Base):
    ...
    # Tracking Pembuatan Akun
    created_by = Column(Integer, ForeignKey("admins.id", ondelete="SET NULL"))
    
    # 🎯 FIX: Menghapus deklarasi ganda created_at yang ada di kode lama
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    last_login_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())

    # 🚨 TAMBAHAN WAJIB V1: Kolom Soft Delete Akun (Menyelaraskan dengan skrip SQL Migration)
